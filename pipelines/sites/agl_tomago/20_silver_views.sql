...
CREATE OR REPLACE VIEW agl_ignition.agl_ot.silver_signals_1m AS
SELECT
  asset_id,
  source_domain,
  signal_name,
...
CREATE OR REPLACE VIEW agl_ignition.agl_ot.silver_signals_latest AS
SELECT * EXCEPT(rn)
FROM (
  SELECT
    event_time,
...
CREATE OR REPLACE VIEW agl_ignition.agl_ot.silver_grid_events AS
SELECT
  event_time,
  asset_id,
  signal_name,
...
-- 4) Maintenance view
CREATE OR REPLACE VIEW agl_ignition.agl_ot.silver_maintenance_events AS
SELECT
  event_time,
  asset_id,
  signal_name,
